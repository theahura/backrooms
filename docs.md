...
- **Survival loop**: the flashlight battery drains over 90 seconds, progressively narrowing the flashlight cone from 100% to 25% of its base angle. Below 25% charge, the flashlight flickers. At 0%, the flashlight goes completely dark. The player must reach the exit zone (in room 0) before the battery dies. Reaching the exit triggers a day-complete transition (camera fadeout + scene restart). Battery math lives in `@/src/systems/battery.js`, following the same pure-function immutable-state pattern as combat and shooting
