...
- **Frame loop**: Phaser calls `GameScene.update()` every frame, which reads input, computes velocity via the movement system, ticks combat/fire/battery state, processes shooting input, resolves E-key interaction (nearest door or switch by distance), checks bullet expiry, runs enemy AI updates (with lit-room freezing), redraws the player/doors/switches/HUD (including inventory counts), and recomputes the flashlight/darkness overlay (including lit rooms). The update loop early-returns if the player is dead or a day-ending transition is in progress
...
- Light switches do not produce wall segments -- they are small interactable objects placed near walls. They interact with the darkness system by drawing white rectangles into the BitmapMask (same mask the flashlight uses) and with the enemy system by providing lit room IDs that freeze/despawn enemies
