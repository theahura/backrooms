...
if (saved) {
  ...
  game.registry.set('weaponInventory', saved.weaponInventory);
}
...
function saveCurrentState() {
  ...
  if (shopState) {
    saveGame(shopState, runCount, game.registry.get('collectedLore') ?? [], game.registry.get('unlockedLocations') ?? ['store'], game.registry.get('activeLocation') ?? 'store', game.registry.get('worldSeed') ?? null, game.registry.get('roomState'), game.registry.get('weaponInventory'));
  }
}
