...
- `@/src/systems/` contains pure functions that scenes call for game math (movement, visibility, room geometry, furniture placement, enemy spawning/AI, combat/health, and shooting/bullets)
...
- **Frame loop**: Phaser calls `GameScene.update()` every frame, which reads input, computes velocity via the movement system, ticks combat/fire cooldowns, processes shooting input, checks bullet expiry, runs enemy AI updates, redraws the player and HUD, and recomputes the flashlight/darkness overlay. The update loop early-returns if the player is dead
