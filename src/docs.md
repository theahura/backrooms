...
index.html
  -> src/main.js (Phaser.Game config, registers both scenes)
       ...
       |    -> src/systems/sprites.js    (pixel art sprite definitions for texture generation)
       |
...
### Things to Know
- **Texture generation lifecycle**: all pixel art textures (furniture, items, bullets, weapon pickups, doors, switches, lore notes) are pre-generated once by `GameScene.generateSpriteTextures()` at the very start of `create()`, before any other creation method runs. This replaces the previous pattern where each creation method (e.g., `createItemTextures()`, `createBullets()`, `createLoreItems()`) generated its own textures inline. Sprite definitions live in `@/src/systems/sprites.js` (pure data, no Phaser dependency); the Phaser `textures.generate()` call happens only in `GameScene`. Furniture rendering uses a sprite-first pattern: standard types (table, shelf, desk, bookcase, bed, armoire, closet, vent) render as Phaser sprites with `setDisplaySize()` to match furniture dimensions, while location-specific types without sprite definitions (e.g., counter, couch from `@/src/systems/locations.js`) fall back to Graphics `fillRect()`. Doors and switches use static sprites whose visibility or texture is toggled per frame, replacing the previous per-frame Graphics clear-and-redraw pattern
