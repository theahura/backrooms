...
- Entry point is `@/src/main.js`, which creates the Phaser.Game instance, loads saved state from localStorage, registers scenes (TitleScene first, so it auto-starts as the boot screen), and installs browser lifecycle listeners for save safety nets
